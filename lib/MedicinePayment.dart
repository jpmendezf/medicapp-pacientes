    ...
    if (isShipping == 1) {
      ...
      if (deliveryDistance < 1) {
        ...
        if (strFinalDeliveryCharge1 == '') {
          var max = listDeliveryCharge
              .reduce((current, next) => int.parse(current.charges!) > int.parse(next.charges!) ? current : next);
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      Text(
                                        ...
                                        style: TextStyle(
                                            fontSize: 16, fontWeight: FontWeight.bold, color: Palette.dark_blue),
                                      ),
                                      '$discountGrandTotal' == "0.0" || '$discountGrandTotal' == "0"
                                          ? Text(
                                              SharedPreferenceHelper.getString(Preferences.currency_symbol).toString() +
                                                  grandTotal.toString(),
                                              style: TextStyle(
                                                  fontSize: 16, fontWeight: FontWeight.bold, color: Palette.blue),
                                            )
                                          : Text(
                                              SharedPreferenceHelper.getString(Preferences.currency_symbol).toString() +
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        ...
                                        style:
                                            TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Palette.blue),
                                      )
                                    ],
                                  ),
                                ],
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) {
    ...
    _paymentToken != "" && _paymentToken!.isNotEmpty
        ...
        : Fluttertoast.showToast(
            msg: getTranslated(context, medicinePayment_paymentNotComplete).toString(),
...
class DeliveryChargesModel {
  ...
  factory DeliveryChargesModel.fromJson(Map<String, dynamic> parsedJson) {
    return DeliveryChargesModel(
        minValue: parsedJson['min_value'], maxValue: parsedJson['max_value'], charges: parsedJson['charges']);
  }
}
